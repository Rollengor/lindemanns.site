...
@if($href)
    <a
        ...
        {{ $attributes->merge(['class' => 'btn d-flex align-items-center justify-content-center gap-3 ' . $btn . ($withLoader ? ' submit-loader' : null) . ($withMiniStyle && $iconName ? ' px-2 px-md-3' : null)]) }}
    >
@else
    <button
        type="{{ $type }}"
