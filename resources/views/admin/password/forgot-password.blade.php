    ...
    <div data-overlayscrollbars-initialize class="w-100 h-100">
        <div class="d-flex flex-column h-100">
            <div class="p-3 p-sm-4 my-auto">
                <form action="{{ route('password.email') }}" method="POST" class="rounded border border-dark border-opacity-25 col-12 col-xs-9 col-sm-7 col-md-5 col-xl-4 col-hd-3 mx-auto shadow-fill">
                    @csrf
                    @method('POST')

                    <!-- heading -->
                    <div class="p-3 border-bottom border-dark border-opacity-25 text-center">
                        <h1 class="fs-4 m-0 lh-sm text-uppercase">{{ __('titles.forgotPassword') }}</h1>
                        <div class="fs-5 text-gray">{{ config('app.name') }}</div>

                        <div class="mt-3">{{ __('texts.forgotPasswordMessage') }}</div>
                    </div>
                    ...
                    <!-- status -->
                    <div data-status="{{ session('status') ? 'sended' : null }}" class="{{ session('status') ? null : 'd-none' }} p-3 text-success border-top border-dark border-opacity-25 text-center">
                        @if (session('status'))
                            <div>{{ session('status') }}</div>
                        @endif
                        <div data-delay-timer class="text-black">{{ __('texts.canResendTimer') }}: <span data-delay-timer-view>00:00</span></div>
                    </div>
                    ...
                    <div class="d-flex flex-column align-items-center gap-2 p-3 pt-4 border-top border-dark border-opacity-25 text-center">
                        <button type="submit" class="btn btn-secondary px-3 submit-loader">
                            <span>{{ __('buttons.emailPasswordResetLink') }}</span>
                        ...
                        <a href="{{ route('login') }}" class="text-decoration-none">{{ __('texts.goToLogin') }}</a>
